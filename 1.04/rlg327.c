...
mon_t *dead_prev(int x, int y, dungeon_t *d) {
  int currMon;
  for(currMon = 0; currMon < d->num_mons - 1; currMon++){
    mon_t *temp = &d->mons[currMon];
    if(x == temp->position[dim_x] && y == temp->position[dim_y]) {
      return temp;
    }
  }
  return NULL;
...
void generate_mon(dungeon_t *d, mon_t *mon, uint32_t intel, uint32_t tele, uint32_t tunn, uint32_t err, int prio){
  ...
  if(!intel && !tele && !tunn && !err){
    mon->c = '0';
    while(1) {
      int currRoom = (rand() % (d->num_rooms - 1)) + 1;
...
void monster_loop(dungeon_t *d){
  ...
  for(currMon = 0; currMon < d->num_mons - 1; currMon++){
    ...
    printf("x: %d, y: %d, %c, prev: %d, %d\n", temp->position[dim_x], temp->position[dim_y], temp->c, temp->prev, currMon);
  }
  ...
  while(1) {
    ...
    if (is == '@') {
      ...
      if(mapxy(removed->position[dim_x], removed->position[dim_y]) != removed->id) {
        mon_t *temp = dead_prev(removed->position[dim_x], removed->position[dim_y], d);
